...
* **Whisper Integration:** Multi-backend speech-to-text supporting OpenAI API, local whisper.cpp CLI, and Python whisper library with automatic fallback.
* **Real-Time Stream Processing:** Chunk-based audio stream processor with built-in VAD, segment extraction, and callback-driven architecture.
...
* **Whisper 语音转写集成:** 多后端支持——OpenAI API、本地 whisper.cpp 命令行、Python whisper 库，具备自动回退机制。
