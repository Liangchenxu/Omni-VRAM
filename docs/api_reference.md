# Omni-VRAM API 参考文档 (API Reference)

本文档涵盖 Omni-VRAM 所有公开 API。版本: `v0.2.0`
...
# 或按模块导入
from vram_core import AudioProcessor, WhisperBridge, WhisperBackend
from vram_core import StreamProcessor, StreamConfig, StreamState
from vram_core import config, OmniConfig, setup_logging
```
...
| 名称 | 类型 | 说明 |
|------|------|------|
| `vram_core.__version__` | `str` | 版本号 |
| `vram_core.CUDA_AVAILABLE` | `bool` | CUDA 扩展是否可用 |

---

## 2. 配置管理 (`vram_core.config`)
...
```python
from vram_core import config  # 全局单例实例
...
| `openai_model` | `str` | `OPENAI_MODEL` | `"whisper-1"` | API 模型名 |
...
| `whisper_threads` | `int` | `WHISPER_THREADS` | `0` | 推理线程数 |
| `whisper_language` | `Optional[str]` | `WHISPER_LANGUAGE` | `None` | 语言代码 |
| `vram_sample_rate` | `int` | `VRAM_SAMPLE_RATE` | `16000` | 目标采样率 |
...
| `update(**kwargs)` | `(Any) -> None` | 运行时更新配置 |
| `print_config()` | `() -> None` | 打印所有配置（隐藏敏感信息） |

### `setup_logging`

```python
from vram_core import setup_logging
...
## 3. 音频处理 (`vram_core.audio_utils`)

### `AudioProcessor`

音频格式处理工具类。所有方法均支持 CPU NumPy 数组，兼容 CUDA 零拷贝管道。
...
| `target_channels` | `int` | `1` | 目标通道数 (1=mono, 2=stereo) |

#### 实例方法

| 方法 | 签名 | 返回值 | 说明 |
|------|------|--------|------|
| `process_file` | `(path, **opts) -> Tuple[ndarray, int]` | `(audio, sample_rate)` | 加载并处理音频文件 |
| `process_bytes` | `(data, format, **opts) -> Tuple[ndarray, int]` | `(audio, sample_rate)` | 处理原始字节数据 |
| `convert_to_target` | `(audio, sr) -> Tuple[ndarray, int]` | `(audio, sample_rate)` | 转换为目标格式 |

#### 静态/类方法

| 方法 | 签名 | 返回值 | 说明 |
|------|------|--------|------|
| `load` | `(path, **opts) -> Tuple[ndarray, int]` | `(audio, sample_rate)` | 从文件加载音频 |
| `load_from_bytes` | `(data, fmt, **opts) -> Tuple[ndarray, int]` | `(audio, sample_rate)` | 从字节加载音频 |
| `detect_format` | `(path) -> str` | `"wav"/"mp3"/...` | 检测文件格式 |
| `stereo_to_mono` | `(audio) -> ndarray` | `mono_audio` | 立体声转单声道 |
| `resample` | `(audio, orig_sr, target_sr) -> ndarray` | `resampled` | 采样率转换 |
| `normalize` | `(audio) -> ndarray` | `normalized` | 峰值归一化 |
| `is_stereo` | `(audio) -> bool` | `True/False` | 判断是否立体声 |
...
| 选项 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `target_sample_rate` | `int` | `None` | 自动重采样到目标采样率 |
| `target_channels` | `int` | `None` | 自动转换通道数 |
| `dtype` | `str` | `"float32"` | 输出数据类型 |
| `mono` | `bool` | `True` | 是否自动转单声道 |

---

## 4. Whisper 转写 (`vram_core.whisper_bridge`)

### `WhisperBackend` (枚举)

```python
from vram_core import WhisperBackend

WhisperBackend.AUTO          # 自动选择最佳可用后端
WhisperBackend.WHISPER_CPP   # whisper.cpp CLI
WhisperBackend.WHISPER_API   # OpenAI Cloud API
WhisperBackend.WHISPER_PYTHON # Python whisper 库
WhisperBackend.NONE          # 无后端（禁用转写）
```

自动选择优先级: `API → CLI → Python → None`
...
result = AudioPreprocessor.prepare_for_whisper(
    audio_data,         # np.ndarray 或 bytes
    source_sr=44100,    # 原始采样率
...
whisper = WhisperBridge(
    ...
    whisper_path=None,              # whisper.cpp 路径（覆盖 .env）
    model_path=None,                # 模型文件路径（覆盖 .env）
    device="cpu",                   # 计算设备
    language="zh",                  # 语言
)
```
...
result = whisper.transcribe(
    ...
    sample_rate=16000,       # 当 audio 为 ndarray 时必须指定
    language="zh",           # 覆盖默认语言
...
## 5. 流处理器 (`vram_core.stream_processor`)
...
```python
from vram_core import StreamConfig

config = StreamConfig(
    sample_rate=16000,           # 采样率 (Hz)
    chunk_duration_ms=100,       # 每块时长 (ms)
    vad_threshold=0.02,          # VAD 能量阈值 (0.0-1.0)
    min_speech_duration_ms=300,  # 最短语音时长 (ms)
    silence_duration_ms=500,     # 判定结束的静音时长 (ms)
    max_segment_duration_s=30.0, # 最大分段时长 (s)
)
```
...
| `vad_threshold` | `float` | `0.02` | VAD 能量阈值 |
| `min_speech_duration_ms` | `int` | `300` | 最短语音时长 |
| `silence_duration_ms` | `int` | `500` | 结束判定静音时长 |
| `max_segment_duration_s` | `float` | `30.0` | 最大分段时长 |
| `chunk_size` | `int` | (自动计算) | 每块样本数 |
| `min_speech_chunks` | `int` | (自动计算) | 最短语音块数 |
| `silence_chunks` | `int` | (自动计算) | 结束判定块数 |

### `StreamState` (枚举)

```python
from vram_core import StreamState
...
```python
from vram_core import StreamProcessor, StreamConfig
...
| 回调 | 签名 | 触发时机 |
|------|------|----------|
| `on_transcription` | `(WhisperResult) -> None` | 转写完成时 |
...
| 方法 | 签名 | 返回值 | 说明 |
|------|------|--------|------|
| `feed` | `(chunk: ndarray) -> None` | `None` | 喂入音频块 |
| `get_state` | `() -> StreamState` | `StreamState` | 获取当前状态 |
| `reset` | `() -> None` | `None` | 重置处理器状态 |

#### `feed` 方法

输入: `np.ndarray`，shape 为 `(chunk_size,)`，dtype 为 `float32`，值域 `[-1.0, 1.0]`。
...
## 6. CUDA 扩展 (`vram_core._vram_hacker`)

> 仅在 CUDA 扩展编译成功且有 NVIDIA GPU 时可用。  
> 检查: `vram_core.CUDA_AVAILABLE`

### `scan_hardware_dna`

```python
info = vram_core.scan_hardware_dna()
...
| `new_tokens` | `torch.Tensor` | 新 token 向量 `(n_tokens, hidden_dim)` |
| `current_pos` | `torch.Tensor` | 当前位置指量 `int32`，原地更新 |
...
| `threshold` | `float` | VAD 阈值 |

| 返回值 | 类型 | 说明 |
|--------|------|------|
| `is_speaking` | `torch.Tensor` | 标量，是否检测到语音 |
| `features` | `torch.Tensor` | 提取的音频特征 |
...
```python
vram_core.inject_into_model(model_handle, tensor_data, offset)
```
...
```python
vram_core.launch_dynamic_kernel(kernel_id, grid, block, *args)
```
...
```python
vram_core.stress_test(size_mb=1024, iterations=100)
```
...
- **dtype**: `float32`
- **值域**: `[-1.0, 1.0]`
- **通道**: 单通道 (mono) 为 1D 数组 `(n_samples,)`
- **立体声**: 2D 数组 `(n_samples, 2)`

### 文件路径

接受 `str` 或 `pathlib.Path` 对象。