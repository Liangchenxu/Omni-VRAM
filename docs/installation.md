...
| **编译器** | MSVC 2019+ (Windows) / GCC 9+ (Linux) | MSVC 2022 / GCC 11+ |
| **RAM** | 8 GB | 16 GB+ |
| **VRAM** | 4 GB | 8 GB+ |
...
# 2. CUDA Toolkit（匹配你的 GPU 驱动版本）

# 设置环境变量（如果 NVCC 不在 PATH 中）
...
print(f"Version: {vram_core.__version__}")
print(f"CUDA Extension: {vram_core.CUDA_AVAILABLE}")

if vram_core.CUDA_AVAILABLE:
    print(vram_core.query_memory())
```
...
### 2.1 方案 A：whisper.cpp（推荐 — 本地、高性能）
...
| 模型 | 文件大小 | 内存占用 | 速度 (CPU) | 准确度 |
|------|----------|----------|------------|--------|
| tiny | ~75 MB | ~400 MB | 最快 | 一般 |
| base | ~142 MB | ~500 MB | 快 | 良好 |
| small | ~466 MB | ~1.0 GB | 中等 | 较好 |
| medium | ~1.5 GB | ~2.6 GB | 较慢 | 好 |
| large | ~3.1 GB | ~4.7 GB | 慢 | 最好 |
...
# .env 中无需额外配置，WhisperBridge 会自动检测
```

首次使用会自动下载模型。推理速度较 whisper.cpp 慢。

### 2.4 自动后端选择

当设置 `WHISPER_BACKEND=AUTO`（默认）时，系统按以下顺序尝试：
...
**Windows 常见问题：** 如果 `pip install pyAudio` 失败，需要安装预编译的 wheel：
...
| `WHISPER_CPP_PATH` | whisper.cpp 可执行文件目录 | — |
...
| `VRAM_SAMPLE_RATE` | 目标采样率 (Hz) | `16000` |
| `VAD_THRESHOLD` | VAD 能量阈值 | `0.02` |
...
**解决：** 确保 CUDA Toolkit 已安装且 `nvcc` 在 PATH 中：
...
**解决：** 安装 [Visual Studio Build Tools](https://visualstudio.microsoft.com/visual-cpp-build-tools/)，勾选 "C++ 桌面开发"。
...
**解决：** 确保 PyTorch 的 CUDA 版本与系统 CUDA Toolkit 版本一致：
...
如果没有 NVIDIA GPU，Omni-VRAM 的核心 CUDA 功能不可用，但 Whisper 转写、音频处理等 Python 模块仍可正常使用（CPU 模式）。

## 下一步

- [快速上手教程](quickstart.md) — 运行第一个示例
