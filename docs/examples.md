...
本目录包含 4 个完整示例应用，展示 Omni-VRAM 在不同场景下的使用方式。

## 快速导航

| 示例 | 场景 | 难度 | 依赖 |
|------|------|------|------|
| [实时语音助手](#实时语音助手) | 实时麦克风录音 + VAD + 转写 | ⭐⭐ | PyAudio |
| [会议录音转写](#会议录音转写) | 长时间录音 + 自动分段 + 导出 | ⭐⭐ | PyAudio, pydub |
| [语音对话机器人](#语音对话机器人) | 多轮语音对话 + 历史管理 | ⭐⭐⭐ | PyAudio |
...
| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `--device` | `int` | 默认设备 | PyAudio 麦克风设备索引 |
| `--save` | `str` | 不保存 | 录音保存文件路径 (WAV) |
| `--threshold` | `float` | `0.02` | VAD 能量阈值 |
...
```
麦克风 → PyAudio → int16 → float32 → StreamProcessor.feed()
                                              ↓
                                     VAD 能量检测
                                      ↓           ↓
                                    静音         语音
...
- 从麦克风录制长时间会议音频
- 静音自动分段（每段最长 30 秒强制切分）
- 每段实时转写并显示
- 会话结束时导出完整转写记录（纯文本 / JSON）
- 支持从文件导入音频进行离线转写
- 自动生成摘要统计
...
| `--input` / `-i` | `str` | 麦克风 | 输入音频文件路径 |
| `--model` | `str` | `base` | Whisper 模型大小 |
| `--language` | `str` | `zh` | 语言代码 |
| `--threshold` | `float` | `0.02` | VAD 阈值 |
| `--verbose` | flag | `False` | 调试模式 |
...
# 设置助手名称
python examples/voice_chat_bot.py --name "小助手"
...
当前机器人使用回显模式（将用户输入原样返回）。要接入真实 LLM，修改 `_generate_response` 方法：
...
- KV-Cache 注入性能测试（8 种配置，torch.cat vs 零拷贝）
...
| 模块 | 测试项 | 指标 |
|------|--------|------|
| `vram_core._vram_hacker` | `append_to_kv_cache` | 延迟(ms)、加速比(x) |
| `vram_core.audio_utils` | `_to_float32`, `stereo_to_mono`, `resample`, `normalize`, `to_wav_bytes` | 处理时间(ms) |
| `vram_core.stream_processor` | `feed` (VAD 静音/语音), 60s 全流程 | 延迟(ms)、实时因子 |
| `vram_core.whisper_bridge` | `transcribe` (tiny/base × 1s/5s/10s) | 处理时间(ms)、实时倍率 |
...
Whisper 本地转写的独立测试脚本。用于快速验证 whisper.cpp 或 Python whisper 后端是否正常工作。
