...
class TestMemoryPressure(unittest.TestCase):
    ...
    def test_values(self):
        self.assertEqual(MemoryPressure.LOW.value, "low")
        self.assertEqual(MemoryPressure.MEDIUM.value, "medium")
        self.assertEqual(MemoryPressure.HIGH.value, "high")
        self.assertEqual(MemoryPressure.CRITICAL.value, "critical")

    def test_all_members(self):
        self.assertEqual(len(MemoryPressure), 4)
...
class TestVRAMStatus(unittest.TestCase):
    ...
    def test_creation(self):
        ...
        self.assertEqual(status.pressure, MemoryPressure.LOW)
    ...
    def test_total_gb(self):
        ...
        self.assertAlmostEqual(status.total_gb, 24.0, places=1)
...
class TestKVCacheEstimate(unittest.TestCase):
    ...
    def test_creation(self):
        ...
        self.assertEqual(est.total_mb, 512.0)
        self.assertEqual(est.n_layers, 32)


class TestVRAMOptimizerInit(unittest.TestCase):
    """Test VRAMOptimizer initialization."""

    def test_default_init(self):
        opt = VRAMOptimizer()
        self.assertEqual(opt.device_id, 0)
        self.assertEqual(opt.cleanup_threshold_pct, 85.0)
        self.assertEqual(opt.target_usage_pct, 60.0)

    def test_custom_init(self):
        opt = VRAMOptimizer(device_id=1, cleanup_threshold_pct=90.0, target_usage_pct=50.0)
        self.assertEqual(opt.device_id, 1)
        self.assertEqual(opt.cleanup_threshold_pct, 90.0)
...
class TestComputePressure(unittest.TestCase):
    ...
    def test_low(self):
        self.assertEqual(VRAMOptimizer._compute_pressure(25.0), MemoryPressure.LOW)
        self.assertEqual(VRAMOptimizer._compute_pressure(49.9), MemoryPressure.LOW)

    def test_medium(self):
        self.assertEqual(VRAMOptimizer._compute_pressure(50.0), MemoryPressure.MEDIUM)
        self.assertEqual(VRAMOptimizer._compute_pressure(69.9), MemoryPressure.MEDIUM)

    def test_high(self):
        self.assertEqual(VRAMOptimizer._compute_pressure(70.0), MemoryPressure.HIGH)
        self.assertEqual(VRAMOptimizer._compute_pressure(84.9), MemoryPressure.HIGH)

    def test_critical(self):
        self.assertEqual(VRAMOptimizer._compute_pressure(85.0), MemoryPressure.CRITICAL)
        self.assertEqual(VRAMOptimizer._compute_pressure(99.0), MemoryPressure.CRITICAL)

    def test_zero(self):
        self.assertEqual(VRAMOptimizer._compute_pressure(0.0), MemoryPressure.LOW)


class TestEstimateKVCache(unittest.TestCase):
    """Test KV-Cache estimation."""

    def test_default_estimate(self):
        est = VRAMOptimizer.estimate_kv_cache()
        self.assertGreater(est.total_mb, 0)
        self.assertEqual(est.n_layers, 32)
        self.assertEqual(est.seq_length, 2048)
        self.assertEqual(est.dtype_bytes, 2)
    ...
    def test_fp16_vs_fp32(self):
        ...
        self.assertAlmostEqual(est_fp32.total_mb, est_fp16.total_mb * 2, places=1)

    def test_batch_size_scaling(self):
        est1 = VRAMOptimizer.estimate_kv_cache(batch_size=1)
        est4 = VRAMOptimizer.estimate_kv_cache(batch_size=4)
        self.assertAlmostEqual(est4.total_mb, est1.total_mb * 4, places=1)
    ...
    def test_seq_length_scaling(self):
        ...
        self.assertAlmostEqual(est_long.total_mb, est_short.total_mb * 4, places=1)

    def test_per_layer_mb(self):
        est = VRAMOptimizer.estimate_kv_cache(n_layers=32)
        expected_total = est.per_layer_mb * 32
        self.assertAlmostEqual(est.total_mb, expected_total, places=1)


class TestRecommendDtype(unittest.TestCase):
...
class TestGetStatus(unittest.TestCase):
    ...
    def test_status_no_gpu(self):
        ...
        self.assertEqual(status.gpu_name, "No GPU")
        self.assertEqual(status.total_mb, 0)
...
class TestCleanupCache(unittest.TestCase):
    ...
    def test_cleanup_no_torch(self):
        ...
        self.assertEqual(opt._cleanup_count, 0)
...
class TestCanAllocate(unittest.TestCase):
    ...
    def test_can_allocate_true(self):
        ...
        with patch.object(opt, 'get_status', return_value=mock_status):
            self.assertTrue(opt.can_allocate(8000))
    ...
    def test_can_allocate_false(self):
        ...
        with patch.object(opt, 'get_status', return_value=mock_status):
            self.assertFalse(opt.can_allocate(8000))


class TestCleanupStats(unittest.TestCase):
    ...
    def test_initial_stats(self):
        ...
        self.assertEqual(stats["last_cleanup_time"], 0.0)
        self.assertEqual(stats["cleanup_threshold_pct"], 85.0)
        self.assertEqual(stats["target_usage_pct"], 60.0)


class TestGetModelSizeEstimate(unittest.TestCase):
    ...
    def test_7b_model_fp32(self):
        ...
        self.assertAlmostEqual(size_mb, 28672.0, delta=1.0)
    ...
    def test_scaling(self):
        ...
        self.assertAlmostEqual(size_fp32, size_fp16 * 2, delta=1.0)
