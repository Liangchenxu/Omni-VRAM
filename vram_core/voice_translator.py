"""
Voice Translator Module for Omni-VRAM
=======================================

Speech-to-Speech translation pipeline:
Speech → ASR → Text Translation → TTS → Translated Speech
...
class VoiceTranslator:
    ...
    def _init_translator(self, backend: str):
        ...
        if backend in ("auto", "marian") and _TRANSFORMERS_AVAILABLE:
            ...
            if model_key in self.MARIAN_MODELS:
                ...
                logger.info("Translation backend: MarianMT for %s→%s", self.source_lang, self.target_lang)
                return

        if backend in ("auto", "google") and _DEEP_TRANSLATOR_AVAILABLE:
            self._backend = "google"
            logger.info("Translation backend: Google Translate for %s→%s", self.source_lang, self.target_lang)
            return
    ...
    def translate_audio(
        ...
        """
        Full audio translation pipeline: ASR → Translate → TTS.
